...
import { WidgetGrid, type GridWidgetConfig } from './WidgetGrid';
import { WidgetWrapper } from './WidgetWrapper';

// ---------------------------------------------------------------------------
// Shared sub-components (copied from Dashboard.tsx to keep this file standalone)
// ---------------------------------------------------------------------------
...
export function DashboardPage() {
  ...
  const noRunsYet = !isLoading && metrics && metrics.runs.runs === 0;

  const tokenUsageContent = isLoading ? (
    <Skeleton className="h-32" />
  ) : noRunsYet ? null : metrics ? (
    <StatCard
      title={
  ...
  const runDurationContent = isLoading ? (
    <Skeleton className="h-32" />
  ) : noRunsYet ? null : metrics ? (
    <StatCard
      title={
  ...
  const costPerTaskContent = taskCost ? (
  ...
  const agentUtilizationContent = utilization ? (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
  ...
  // Build grid widget array
  const gridWidgets: GridWidgetConfig[] = [
    {
      id: 'token-usage',
      visible: widgets.showTokenUsage,
      content: (
        <WidgetWrapper title="Token Usage" onViewAll={() => setDrillDown('tokens')}>
          {tokenUsageContent ?? <span className="text-muted-foreground text-sm">No data</span>}
        </WidgetWrapper>
      ),
    },
    {
      id: 'run-duration',
      visible: widgets.showRunDuration,
      content: (
        <WidgetWrapper title="Run Duration" onViewAll={() => setDrillDown('duration')}>
          {runDurationContent ?? <span className="text-muted-foreground text-sm">No data</span>}
        </WidgetWrapper>
      ),
    },
    {
      id: 'agent-comparison',
      visible: widgets.showAgentComparison,
      content: (
        <WidgetWrapper title="Agent Comparison">
    ...
    {
      id: 'status-timeline',
      visible: widgets.showStatusTimeline,
      content: (
        <WidgetWrapper title="Agent Activity">
          <StatusTimeline />
        </WidgetWrapper>
      ),
    },
    ...
    {
      id: 'wall-time',
      visible: widgets.showWallTime,
      content: (
        <WidgetWrapper title="Wall Time">
    ...
    {
      id: 'session-metrics',
      visible: widgets.showSessionMetrics,
      content: (
        <WidgetWrapper title="Session Metrics">
    ...
    {
      id: 'activity-clock',
      visible: widgets.showActivityClock,
      content: (
        <WidgetWrapper title="Activity Clock">
    ...
    {
      id: 'where-time-went',
      visible: widgets.showWhereTimeWent,
      content: (
        <WidgetWrapper title="Where Time Went">
    ...
    {
      id: 'hourly-activity',
      visible: widgets.showHourlyActivity,
      content: (
        <WidgetWrapper title="Hourly Activity">
    ...
    {
      id: 'trends-charts',
      visible: widgets.showTrendsCharts,
      content: (
        <WidgetWrapper title="Historical Trends">
